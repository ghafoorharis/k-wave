...
base_folder = 'B_Mode_Simulation_Dataset_New';
...
exp_num = 0;
for xy_pair = 1:num_pairings
    shift_pair_vector = pairings(xy_pair,:);
    x_shift = shift_pair_vector(1);
    y_shift = shift_pair_vector(2);
    x_pos = center_X + x_shift;
    y_pos = center_Y + y_shift;
    for rad = 1:length(range_radii)
        radius = range_radii(rad);
        % =========================================================================
        ...
        [sound_speed_map, density_map, scattering_circle] = define_phantom( ...
                                ...
                                x_pos, ...
                                y_pos, ...
                                constant_scattering_region_add, ...
                                constant_scattering_region_multiplier, ...
                                constant_scattering_region_divider);
        %=======================================================================
        % RUN THE SIMULATION
        % ===================================================================
        if RUN_SIMULATION
            ...
            % disp('s')
            load_results_path =fullfile(base_folder,"experiment_1_results");
            % Load scan lines from file if the simulation is turned off
            loaded_data = load(fullfile(load_results_path, sprintf('scan_lines_experiment_%d.mat', ...
                1)),'scan_lines');
            % break;
