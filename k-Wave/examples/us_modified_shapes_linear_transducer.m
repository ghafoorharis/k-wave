...
% ---- Define Shapes ----
% 1. Circle (Sphere in 3D)
x_shift = [0,2,-2,3,-3,4,-4,6,-6].* 1e-3;
y_shift = [0,2,-2,3,-3,-4,4,6,-6] .* 1e-3;
num_pairings = length(x_shift) * length(y_shift);
pairings = zeros(num_pairings, 2);
count = 1;
for i = 1:length(x_shift)
    for j = 1:length(y_shift)
        pairings(count, :) = [x_shift(i), y_shift(j)];
        count = count + 1;
    end
end
% Shuffle the rows to get random unique pairings
% shuffled_pairings = pairings(randperm(size(pairings, 1)), :);

idx = 1;
shift_pair_vector = pairings(idx,:);
radius = 6e-3;  % [m]
% x_pos = 24e-3 + shift_pair_vector(1);  % [m]
% y_pos = 27e-3 + shift_pair_vector(2);  % [m]
x_pos = 24e-3 + 6e-3;
y_pos = 27e-3 + 6e-3;
...
% x_pos = 15e-3;  % [m]
% y_pos = 20e-3;  % [m]
% scattering_square = makeCube(Nx_tot, Ny_tot, Nz_tot, round(x_pos/dx), round(y_pos/dx), Nz_tot/2, round(cube_size/dx));
% sound_speed_map(scattering_square == 1) = scattering_c0(scattering_square == 1);
% density_map(scattering_square == 1) = scattering_rho0(scattering_square == 1);


% % 3. Triangle (Pyramid in 3D)
% pyramid_size = 10e-3; % [m]
% x_pos = 35e-3;  % [m]
% y_pos = 40e-3;  % [m]
% 
% scattering_triangle = makePyramid(Nx_tot, Ny_tot, Nz_tot, round(x_pos/dx), round(y_pos/dx), Nz_tot/2, round(pyramid_size/dx));
% sound_speed_map(scattering_triangle == 1) = scattering_c0(scattering_triangle == 1);
% density_map(scattering_triangle == 1) = scattering_rho0(scattering_triangle == 1);


% ---- Assign Acoustic Properties ----
% % Modify sound speed and density for each shape
% sound_speed_map(scattering_circle == 1) = c0;  % Different sound speed for circle
% density_map(scattering_circle == 1) = rho0;
% 
% sound_speed_map(scattering_square == 1) = c0;  % Different sound speed for square
% density_map(scattering_square == 1) = rho0 - 50;
% 
% sound_speed_map(scattering_triangle == 1) = c0 ;  % Different sound speed for triangle
% density_map(scattering_triangle == 1) = rho0;

% You can now use sound_speed_map and density_map for your simulation
...
if RUN_SIMULATION
    ...
    save example_us_bmode_scan_lines_modified scan_lines;
    
else
    
    % load the scan lines from disk
    load example_us_bmode_scan_lines_modified;
...
leeway = 0;
x = 1;
scan_lines_fund = gaussianFilter(scan_lines, 1/kgrid.dt, 1/x * tone_burst_freq,100 + leeway, true);
set(gca, 'XLim', [0, 6]);
scan_lines_harm = gaussianFilter(scan_lines, 1/kgrid.dt, 2 * 1/x * tone_burst_freq,30 + leeway, true);
set(gca, 'XLim', [0, 6]);
...
% 
% % physical properties of the transducer
% transducer_plot.number_elements = 32/sc + number_scan_lines - 1;
% transducer_plot.element_width = 2;
% transducer_plot.element_length = 24/sc;
...
% voxelPlot(single(transducer_plot.active_elements_mask | scattering_circle));
% 
% view(26, 48);
