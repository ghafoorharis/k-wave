...
% Circle parameters for Bézier curve
center_x = 54;  % X position of the circle center
center_y = 75;  % Y position of the circle center
center_z = 27;  % Z position of the circle center
radius = 20;    % Radius of the circle in grid points

% Define the control points for the 3D Bézier curve (circular arc)
P0 = [center_x + radius, center_y, center_z];     % Start point on the circle (right side)
P2 = [center_x, center_y - radius, center_z];     % End point on the circle (top side)
P1 = [center_x - radius, center_y - radius / 2, center_z];  % Control point to form the arc

% % Create a matrix of control points (3x3 matrix)
% P = [P0'; P1'; P2'];

% Parameter t for interpolation (from 0 to 1)
t = linspace(0, 1, 100);  % 100 points along the Bézier curve

% Compute the points along the Bézier curve
% bezier_curve = makeCircularBezier(P, t);

% Generate the scattering region around the Bézier curve
physical_radius = 0.005;  % 1 cm
...
% Modify sound speed and density in the scattering region
% medium.sound_speed = scattering_map;  % Increase sound speed in scatterers
% medium.density = scattering_map;    % Increase density in scatterers

% =========================================================================
% RUN THE SIMULATION
% =========================================================================

% preallocate the storage for scan lines
...
if RUN_SIMULATION
    ...
    for scan_line_index = 1:number_scan_lines
        ...
        % update medium position for the next scan line
        medium_position = medium_position + transducer.element_width;
    end

    % save the scan lines to disk
    save example_us_bmode_scan_lines_modified_bezier scan_lines;
else
    % load the scan lines from disk
    load example_us_bmode_scan_lines_modified_bezier;
end
